...
function qconv_32_kernel!(output, input, kernel, channels_in_kernel)
    ...
    input_cache = @cuDynamicSharedMem(UInt32, (P, Wₖ, Hₖ, N_var))
    #input_cache = @cuDynamicSharedMem(UInt32, (3, 3, 3))

    #input_cache[1, 1, 1] = 1
    
    kernel_copy!(input_cache,
                 input, (1:P, w_left+1:w_left+Wₖ, h_top+1:h_top+Hₖ, n_min:n_max))
            ...
            for p in 1:P
                for x in 1:Hₖ
                    for y in 1:Wₖ
                        ...
                        input_value = input_cache[p, x, y, n_i]
